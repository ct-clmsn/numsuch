module Core {
  ...
  class LabelMatrix {
    ...
    /*
      input: remove Sampling probablilty.
      returns: LabeledMatrix with labels set to 0 with prob = p
     */
    proc randomSubSample(L: LabelMatrix, remove: real) {
      var Yout = L;
      var ps: [ldom] real;
      fillRandom(ps);
      for l in ldom{
  ...
  /*
   Does the pairwise cosine similarity between rows of X.
   Dimensions must be conformable
   */
  proc cosineSimilarity(X:[?Xdom], denseOutput=true) {
    // TODO
    if !denseOutput then halt('denseOutput=false not yet supported');

    var cosSimDom: domain(2) = {Xdom.dim(1), Xdom.dim(1)},
        cosSim: [cosSimDom] real;
  ...
  /*
   ...
   Dimensions must be conformable
   */
  proc cosineSimilarity(X:[?Xdom], Y:[?Ydom], denseOutput=true) {
    // TODO
    if !denseOutput then halt('denseOutput=false not yet supported');
    ...
    var cosSimDom: domain(2) = {Xdom.dim(1), Ydom.dim(1)},
        cosSim: [cosSimDom] real;
